...
public class ContainerFrier extends Container
{
	...
	public ContainerFrier(InventoryPlayer invPlayer, TileEntityFrier cabinet)
	{
		...
		this.addSlotToContainer(new Slot(cabinet.getInventory(), 0, 22, 35));

		for (i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				this.addSlotToContainer(new Slot(cabinet.getInventory(), 1 + i + j * 4, 71 + 18 * i, 17 + 18 * j)
				{
					@Override
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2)
	{
		...
		if (slot != null && slot.getHasStack())
		{
			...
			} else if (!this.mergeItemStack2(itemstack1, 0, 10, false))
			{
				return null;
			}
