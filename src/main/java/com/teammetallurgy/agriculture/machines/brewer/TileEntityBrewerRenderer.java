...
public class TileEntityBrewerRenderer extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(final TileEntityBrewer tileentity, final double x, final double y, final double z, final float partialTickTime)
    {
        int direction = 0;
        float yOffset = 0;
        ...
        final int rotation = TileEntityBrewerRenderer.rotations[direction];
        ...
        final FluidStack fluid = tileentity.getRightTank().getFluid();
        // int color = tileentity.getRightTank().
        if (fluid == null || fluid.amount <= 0) { return; }

        final int[] displayList = FluidRender.getFluidDisplayLists(fluid, tileentity.getWorldObj(), false);
